    ...
    return (
      <div id="loginForm" className="ui container topMargin72px">
        ...
        <form onSubmit={handleSubmit(handleSignUp)} className="ui form maxWidth centerDiv topMargin24px">
          ...
          <Field component={Input} type="password" id="confirmPassword" name="confirmPassword" label="Confirm Password" />
...
function validate({ email, password, confirmPassword, firstname, lastname}) {
  ...
  const validPassword = /^(?=.*[0-9])(?=.*[!@#$%^&*])(?=.*[a-zA-Z])[a-zA-Z0-9!@#$%^&*]{6,32}$/;
  const validName = /^[a-zA-Z ,.'-]{1,50}$/;
 
  if (!validEmail.test(email)) errors.email = 'Please enter a valid email';
  if (!validPassword.test(password)) errors.password = 'Password must be between 6 and 32 characters, at least 1 letter, 1 number, and 1 special character';
  if (password !== confirmPassword) errors.confirmPassword = 'Confirm password does not match password';
